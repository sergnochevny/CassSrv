...
begin
  ...
  try
    ...
    on e: Exception do begin
      __DataStream.__EventLogger.LogError('550_DataStreamThreadProc exception: '+e.Message);
      Result := $FFFFFFFF;
    end;
  end;
  ExitThread(Result);
                    ...
                    try
                      ...
                      if GotoKey then begin
                        ...
                        try
                          ...
                          on e: Exception do begin
                            __EventLogger.LogError('550_TDataStream.__GetGoodsParam exception 1: '+e.Message);
                            Cancel;
                          end;
                        end;
                      end;
                      ...
                      on e: Exception do begin
                        __EventLogger.LogError('550_TDataStream.__GetGoodsParam exception 2: '+e.Message);
                        Result := False;
                      end;
                    end;  
                  end; 
...
{$ifndef CS}
      ...
      __EventLogger.LogError('550_TDataStream.__GetGoodsParam __EvH timeout');
{$endif}  
  finally
{$ifdef CS}
...
begin
  ...
  while (true and assigned(_tmp)) do begin
    try
      ...
      if ( FindFirst(__TmpDir+IntToStr(_tmp^.SerialNum)+__tmpast+__tmpext, faAnyFile, _sr) = 0) then begin
        try
          ...
          try
              ...
              if _cds.FieldByName('Operation').AsInteger in 
                ...
                if not assigned(TCheckState(_tmp^.CheckState).__CheckData) then begin
                  ...
                  try
                    ...
                    on e: Exception do begin
                      __EventLogger.LogError('550_TClientDataSet(TCheckState(_tmp^.CheckState).__CheckData).AppendData exception 1: '+e.Message);
                    end;
                  end;                    
                  _cds.Close;
                  _cds.Free;
            ...
            on e: Exception do begin
              __EventLogger.LogError('550__TDataStream.__CheckTmpCDS loading data file:'+__TmpDir+_sr.Name+' exception: '+e.Message);
              _Log.WriteToLog(PChar(DateTimeToStr(Now)+': - '+_sr.Name+' '+DateTimeToStr(FileDateToDateTime(FileAge(__TmpDir+_sr.Name)))),
                              Length(DateTimeToStr(Now)+': - '+_sr.Name+' '+DateTimeToStr(FileDateToDateTime(FileAge(__TmpDir+_sr.Name)))));
              _cds.Close;
              _cds.Free;
          ...
          on e: Exception do begin
            __EventLogger.LogError('550_TDataStream.__CheckTmpCDS exception 1: '+e.Message);
          end;
        end;
        while (FindNext(_sr) = 0) do begin
          try
                ...
                if _cds.FieldByName('Operation').AsInteger in 
                  ...
                  if not assigned(TCheckState(_tmp^.CheckState).__CheckData) then begin
                    ...
                    try
                      ...
                      on e: Exception do begin
                        __EventLogger.LogError('550_TClientDataSet(TCheckState(_tmp^.CheckState).__CheckData).AppendData exception 2: '+e.Message);
                      end;
                    end;                    
                  end;
                end;                
            ...
            on e: Exception do begin
              __EventLogger.LogError('550_TDataStream.__CheckTmpCDS exception 2: '+e.Message);
            end;
          end;
        end;
        FindClose(_sr);
      ...
      on e: Exception do begin
        __EventLogger.LogError('550_TDataStream.__CheckTmpCDS exception: '+e.Message);
      end;
    end;
  end;
end;
...
begin
  ...
  //fields  
    ...
    on E:Exception do begin
      __EventLogger.LogError('550_TDataStream.__GoodsCasheCreate exception: '+e.Message);
      Result := nil;
    end;
  end;
end;
...
{$endif}
    ...
    on E: Exception do begin
      __EventLogger.LogError('550_TDataStream.__OpenGoodsCashe exception: ');
      Result := nil;
    end;
  end;
end;
  ...
  try
    try
      ...
      case _opType of
        ...
        opRejectAnnul: __RejectLastOperation(_P, _OpData^.__DevEnum);
      ...
      on e: Exception do begin
        __EventLogger.LogError('550_TDataStream.ProcessPacket exception: '+e.Message);
      end;
    end;
  finally
...
{$endif}  
      try
        ...
        on e: Exception do begin
          __EventLogger.LogError('550_TDataStream.__RefreshGoodsCashe exception: '+e.Message);
        end;
      end;
{$ifndef CS}
    end
    else
      __EventLogger.LogError('550_TDataStream.__RefreshGoodsCashe __EvH timeout');
{$endif}  
  finally
{$ifdef CS}
    LeaveCriticalSection(__CS);
...
//            if (_msg.hwnd = 0) then
//              __EventLogger.LogInformation('550_TDataStream.Run _msg.hwnd: '+IntToStr(_msg.hwnd)+'; _msg.message: '+IntToStr(_msg.message)+'; _msg.wParam: '+IntToStr(_msg.wParam));
            if (_msg.hwnd = 0) then
      ...
      on E: Exception do begin
        __EventLogger.LogError('550_TDataStream.Run exception: '+E.Message);
        __Terminated := True;
      end;
    end;
  finally
...
begin
  ...
  __EventLogger := TEventLogger.Create('550_CassSrv DataStream '+ IntToStr(Integer(Pointer(__ConnectionStream))));
...
// Сообщение о продаже запрограммированного товара.
  try
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__SaleArt exception: '+e.Message);
    end;
  end;
end;
...
// Сообщение об оплате.
  try
    ...
    if (Abs(_Sum) > Abs(_TotalSum)) then _Sum := _TotalSum;
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__Payment exception: '+e.Message);
    end;
  end;
end;
...
// Запрос на аннулирование всего чека.
  try
    ...
    if TCheckState(_DevEnum^.CheckState).__tRejectCheckAnnul then begin
      _ProcStr := '';
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__Annul exception: '+e.Message);
    end;
  end;
end;
...
begin
// Подтверждения на аннулирование всего чека.
  try
...
begin
//Обработка запроса скидки/надбавки
  Result := 0.00;
  try
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__ParseQueryDisc exception: '+e.Message);
    end;
  end;
end;
...
//Сообщение о скидке/надбавке.
  try
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__QueryDisc exception: '+e.Message);
...
var
  ...
  _ChkNum: Integer;
begin
// Запрос на продажу товара по коду.
  try
    _S_Info := PInfo(_P);
    _ProcStr := Trim(PChar(TBuffer(_S_Info^.BigReadBuffer)));
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__SaleCode exception: '+e.Message);
      TCheckState(_DevEnum^.CheckState).__tReject := False;
      _ProcStr := #10+';;;;;;;;;;;'+#13;
      _S_Info^.BigWriteCount := Length(_ProcStr);
      __move(_ProcStr, _S_Info^.BigWriteBuffer, _S_Info^.BigWriteCount);
...
//Сообщение о скидке/надбавке.
  try
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__Discount exception: '+e.Message);
    end;
  end;
end;
...
{$endif}  
      try
                  ...
                  try
                    ...
                    if GotoKey then begin
                      ...
                      try
                        ...
                        on E:Exception do begin                      
                          __EventLogger.LogError('550_TDataStream.__ParseBar exception 1: '+e.Message);                        
                          Cancel;
                        end;
                      end;
                    end;
                    ...
                    on E:Exception do begin                      
                      __EventLogger.LogError('550_TDataStream.__ParseBar exception 2: '+e.Message);                        
                      Result := False;
                    end;
                  end;  
                end; 
        ...
        on e: Exception do begin
          __EventLogger.LogError('550_TDataStream.__ParseBar exception: '+e.Message);
        end;
      end;
{$ifndef CS}
    end
    else
      __EventLogger.LogError('550_TDataStream.__ParseBar __EvH timeout.');
{$endif}  
  finally
{$ifdef CS}
    LeaveCriticalSection(__CS);
...
var
  ...
  _ChkNum: Integer;
begin
// Запрос на продажу товара по штрихкоду.
  try
    _S_Info := PInfo(_P);
    _ProcStr := Trim(PChar(TBuffer(_S_Info^.BigReadBuffer)));
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__SaleBar exception: '+e.Message);
      TCheckState(_DevEnum^.CheckState).__tReject := False;
      _ProcStr := #10+';;;;;;;;;;;'+#13;
      _S_Info^.BigWriteCount := Length(_ProcStr);
      __move(_ProcStr, _S_Info^.BigWriteBuffer, _S_Info^.BigWriteCount);
...
// Запрос по ключу.
  try
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__QueryKey exception: '+e.Message);
    end;
  end;
end;
...
// Отказ на продажу товара после ответа на запрос продажи.
  try
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__RejectSaleCode exception: '+e.Message);
    end;
  end;
end;
...
// Отказ на продажу товара после ответа на запрос продажи.
  try
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__RejectSaleBar exception: '+e.Message);
    end;
  end;
end;
...
begin
  try
    _S_Info := PInfo(_P);
    TCheckState(_DevEnum^.CheckState).RejectLastOperation(_DevEnum);
  except
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__RejectLastOperation exception: '+e.Message);
    end;
  end;
end;
...
{$endif}    
    ...
    on e: Exception do begin
      __EventLogger.LogError('550_TDataStream.__GetParams exception: '+e.Message);
      Result := False;
    end;
  end;
end;
