...
type
  ...
  public
    ...
    procedure RejectLastOperation(_DevEnum: PDevEnum);
...
begin
  if __tReject and assigned(__CheckData) then begin
    if __CheckData.RecordCount > 0 then begin
      ...
      if ((((not _P.isBarCode) and (__CheckData.FieldByName('Code').AsInteger = StrToInt(_P.Code))) or 
          (_P.isBarCode and (__CheckData.FieldByName('BarCode').AsString = _P.BarCode))) and
          (abs(__CheckData.FieldByName('Count').AsFloat + StrToFloat(_P.Count)) < 0.001)) then begin
        __CheckData.Delete;
        try
          __CheckData.SaveToFile(__FileName);
...
procedure TCheckState.RejectLastOperation(_DevEnum: PDevEnum);
var
  _SerialNum: Integer;
  _tmp: String;
begin
        ...
        try
          if not ((not BOOL(__SaveServiceCheck)) and (( _tKnd = __kndNullCheck ) or
            ...
            if ( (( _tKnd <> __kndNullCheck ) and
               ...
               (Trunc( (Abs(_Sum) - Abs(_TotalSum))*10000+0.5 )/100 >= 0 )) or 
