...
var
  _SQL: String;
begin
  _SQL := __select__+__all__+__from__+'"BarCodes"';
...
var
  _SQL: String;
begin
  _SQL := __select__+__all__+__from__+'"DepPrice"';
  if Length( _where )>0 then
    _SQL := _SQL + __where__ + _where;
  SelectData( Result, _SQL );
...
var
  _SQL: String;
begin
  _SQL := __select__+__all__+__from__+'"Ecrs"';
  if Length( _where )>0 then
    _SQL := _SQL + __where__ + _where;
  SelectData( Result,  _SQL );
end;
...
var
  _SQL: String;
begin
  _SQL := __select__+__all__+__from__+'"Goods"'+__where__+'DISABLED <> ''T''';
  if Length( _where )>0 then
